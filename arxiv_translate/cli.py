...
from .tex import discover_main_tex
from .translator import TranslationCache, prepare_translated_tree, translate_tex_tree
...
def run(args: argparse.Namespace) -> int:
    ...
    api_key = config_string(config, "deepseek_api_key")
    model = config_string(config, "deepseek_model")
    guide_model = config_string(config, "deepseek_guide_model")
    ...
    while True:
        ...
        if char in {"\x00", "\xe0"}:
            ...
            if key == "H" and history_index > 0:
                history_index -= 1
                buffer, line_length = _redraw_interactive_line(
                    prompt, history[history_index], line_length
                )
            elif key == "P":
                if history_index < len(history) - 1:
                    ...
                    next_buffer = ""
                buffer, line_length = _redraw_interactive_line(
                    prompt, next_buffer, line_length
                )
            continue
        if char == "\x03":
...
def _redraw_interactive_line(
    ...
    print(f"\r{line}{padding}\r{line}", end="", flush=True)
    return buffer, len(line)
